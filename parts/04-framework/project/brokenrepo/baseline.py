...
Every line here is justified somewhere in Parts 0 through IV. The eleven bugs
in bugs/ are each this file with exactly one thing changed.

Deliberately small and CPU-only: the point is to run it dozens of times in an
afternoon, not to train anything impressive.
"""
import torch
...
class MLP(nn.Module):
    ...
    def __init__(self, d_in=20, hidden=64, n_classes=2, depth=2, p_drop=0.3):
        ...
        self.layers = nn.ModuleList()            # a list that is also a Module
        prev = d_in
